...
export default function HomePage() {
  ...
  const refresh = async () => {
    ...
    const withThumbs = await Promise.all(
      list.map(async (p) => {
        const thumb = await idbGetThumb(p.id).catch(() => undefined);
        return { ...p, thumb };
      })
    );
  ...
  const openProject = async (id: string) => {
    ...
    if (loaded) {
      createProject({ id, name: (loaded as any).name, width: (loaded as any).width, height: (loaded as any).height, fps: (loaded as any).fps, format: (loaded as any).format, tracks: (loaded as any).tracks, narration: (loaded as any).narration, duration: (loaded as any).duration });
      router.push("/editor");
    }
  };
  ...
  const duplicateProject = async (id: string, e: React.MouseEvent) => {
    ...
    if (!proj) return;
    const copy = { ...structuredClone(proj), id: Math.random().toString(36).slice(2, 10) + Date.now().toString(36), name: `${proj.name} (copy)`, updatedAt: Date.now() };
  ...
  const fmtDur = (s: number) => (s >= 60 ? `${Math.floor(s / 60)}:${String(Math.floor(s % 60)).padStart(2, "0")}` : `0:${String(Math.max(0, Math.floor(s))).padStart(2, "0")}`);
  ...
  return (
    <div className="page">
      ...
      <main className="container">
        ...
        <input ref={fileInput} type="file" accept=".json,.vidforge.json" hidden onChange={(e) => { const f = e.target.files?.[0]; if (f) importProject(f); e.target.value = ""; }} />
